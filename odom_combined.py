...
def doOdomPub():
    ...
    if begin_flag:
        ...
        car_lastPose = car_currentPose
    if camUpdateFlag:
        Tad = np.array([cam_currentPose.position.x,
                        cam_currentPose.position.y, cam_currentPose.position.z])
        q = [cam_currentPose.orientation.x, cam_currentPose.orientation.y,
        ...
        if carUpdateFlag:
            ...
            q = [Odom_last.pose.pose.orientation.x, Odom_last.pose.pose.orientation.y,
                 Odom_last.pose.pose.orientation.z, Odom_last.pose.pose.orientation.w]
            euler = euler_from_quaternion(q)
            theta_last = euler[2]

            delta_car = math.sqrt((car_currentPose.position.x - car_lastPose.position.x) * (car_currentPose.position.x - car_lastPose.position.x) + (
                car_currentPose.position.y - car_lastPose.position.y) * (car_currentPose.position.y - car_lastPose.position.y))
            if car_currentPose.position.z < 0.000001:
                delta_car = -1. * delta_car
            Odom.pose.pose.position.x = Odom_last.pose.pose.position.x + \
                delta_car * math.cos(theta_last)
            Odom.pose.pose.position.y = Odom_last.pose.pose.position.y + \
                delta_car * math.sin(theta_last)
            Odom.pose.pose.position.z = 0.

            q1 = [car_lastPose.orientation.x, car_lastPose.orientation.y,
                  car_lastPose.orientation.z, car_lastPose.orientation.w]
            q2 = [car_currentPose.orientation.x, car_currentPose.orientation.y,
        ...
        car_diff = abs(car_currentPose.position.x - car_lastPose.position.x) + \
            abs(car_currentPose.position.y - car_lastPose.position.y)
    ...
    Odom.twist.twist.linear.x = 30 * \
        (Odom.pose.pose.position.x - Odom_last.pose.pose.position.x)
    Odom.twist.twist.linear.y = 30 * \
        (Odom.pose.pose.position.y - Odom_last.pose.pose.position.y)
    ...
    q1 = [Odom_last.pose.pose.orientation.x, Odom_last.pose.pose.orientation.y,
          Odom_last.pose.pose.orientation.z, Odom_last.pose.pose.orientation.w]
    euler1 = euler_from_quaternion(q1)
    ...
    M = tf.transformations.quaternion_matrix(q)
    Rbc = M[:3, :3]
    Tbc = np.array([Odom.pose.pose.position.x,
