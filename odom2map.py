...
def doTfPub():
    ...
    if camUpdateFlag and abs(time0_diff.to_sec()) < 0.05:
        ...
        Mof = np.identity(4)
        q = [car_currentPose.orientation.x, car_currentPose.orientation.y,
             car_currentPose.orientation.z, car_currentPose.orientation.w]
        M = tf.transformations.quaternion_matrix(q)
        Rfo = M[:3, :3]
        Tfo = np.array([car_currentPose.position.x,
                        car_currentPose.position.y, car_currentPose.position.z])
        Mof[:3, :3] = Rfo.T
